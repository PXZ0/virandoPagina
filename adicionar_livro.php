...
<html lang="pt-br">
	<head>
		...
		<meta charset="UTF-8">
		<title>Adicição de um livro</title>
	...
	<body>
		<main class="bg">
			<div class="box-add rounded">
				...
				<hr>
				<form name="adc_livro" action="#" method="POST" enctype="multipart/form-data">
					<section class="form-group">
						<label for="titulo">Titulo do livro</label>
						<input id="titulo" class="form-control" type="text" name="titulo" autofocus required>
					</section>
					<section class="form-group">
					...
					<input type="submit" class="btn btn-success" name="registro_livro" value="Registrar Livro">
				</form>
				<div class="link">
				...
				<?php
					if(isset($_POST['registro_livro'])){
						...
						if(!empty($capa)){
							...
							$sqlin='insert into livros(titulo, autor, andamento, id_usuario, foto_capa) values("'.$titulo.'","'.$autor.'",'.$andamento.','.$_SESSION['id_usuario'].',"'.$nome_capa.'");';
						}else{
							$sqlin='insert into livros(titulo, autor, andamento, id_usuario) values("'.$titulo.'","'.$autor.'",'.$andamento.','.$_SESSION['id_usuario'].');';
						}
						
						$inserir=mysqli_query($conexao, $sqlin);

						if($inserir){
							echo('<script>window.alert("Livro cadastrado com sucesso!"); window.location="biblioteca.php"; </script>');
						}else{
							echo('<script>window.alert("Houve um erro ao adicionar o livro!"); window.location="adicionar_livro.php"; </script>');
						}
					}
				?>
			</div>
